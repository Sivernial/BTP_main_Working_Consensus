...
class Block:
    ...
    def from_dict(cls, block_dict):
        new_block = cls(block_dict['timestamp'], block_dict['data'], block_dict['previous_hash'], block_dict['random_number'])
...
class BlockChain:
    def __init__(self, name, hashPower, init_blocks_file):
        ...
        self.ghast_weights = {}
        self.longest_chain_hash = "0038e8a8063ea575068bb0ccebe242a42ba2c1e45204e4f37bb87ea8364cfa9e"
        self.name = name
            ...
            if not ghast_enabled:
                for child in self.graph[init]:
                    if self.subtree_size[child] > maxa_subtree_size:
                        ...
                        maxa_node = child
                    elif self.subtree_size[child] == maxa_subtree_size and child < maxa_node:
                        maxa_node = child
            ...
            if maxa_subtree_size <= total_subtree_size/2 or ghast_enabled:
                ...
                for child in self.graph[init]:
                    if self.ghast_weights[child] > maxa_subtree_size:
                        ...
                        maxa_node = child
                    elif self.ghast_weights[child] == maxa_subtree_size and child < maxa_node:
                        maxa_node = child            
    ...
    def validate_block(self, block):
        ...
        if block.hash not in self.height_map and block.previous_hash in self.height_map and Block.check_difficulty(block.hash) >= 2:
            return True
        ...
        print(f"Block previous hash in height map: {block.previous_hash in self.height_map}")
    ...
    def write_block_to_csv(self, block:Block):
        with open(self.csv_file, 'a') as f:
            f.write(f'{block.timestamp},{block.previous_hash},{block.hash},{block.random_number},{self.owner_map[block.hash]}\n')
